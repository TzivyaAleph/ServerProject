...
#return ip's of the URL
def handle_url(url_srt):
    data2 = ""
    if validators.url('http://{}'.format(url_srt)):
        dns_query = IP(dst='8.8.8.8') / UDP(dport=53) / DNS(rd=1, qd=DNSQR(qname=url_srt))
        res = sr1(dns_query, verbose=0, timeout=2)
        for i in range(res[DNS].ancount):
            data2 += str(res[DNS].an[i].rdata) + "\n"
    else:
        data2 += 'Error: not a valid url address'
    return data2
...
def sending_options(data1, clients, my_socket):
    ...
    if data1[0] == "NSLOOKUP":
        if data1[1] in clients.keys():
                str_ip = "".join(clients[data1[1]].getsockname()[0])
                return str_ip, my_socket
        else:
            string_url = data1[1]
            ip_address_string = handle_url(string_url)
            return ip_address_string, my_socket
    if data1[0] == "EXIT":
        return "", my_socket
...
# a dict with the sockets and ip
my_clients_IP = {}
# a dict with the names and the sockets
my_clients = {}
while True:
    ...
    for current_socket in rlist:
        if current_socket is server_socket:
            ...
            #my_clients_IP[connection] = client_address[0]
            print("New client joined!", client_address)
            client_sockets.append(connection)
            print_client_sockets(client_sockets)
        else:
            is_valid, data = protocol_test.get_msg(current_socket)
            if is_valid:
                if data == "EXIT\r":
    ...
    for message in messages_to_send:
        ...
        if current_socket in wlist:
            response = protocol_test.create_msg(data)
